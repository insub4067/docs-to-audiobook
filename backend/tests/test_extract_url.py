...
리스크다. _pin_safe_public_host는 실제 소켓 조회(127.0.0.1 등 리터럴은
네트워크 없이 즉시 풀린다)로, 나머지는 requests.get을 패치해 실제
네트워크 호출 없이 검증한다.

patch 대상은 routes.extract_url이다 — 이 라우트가 직접 호출하는
이름들(require_user_id, _pin_safe_public_host, requests.get)은 전부 이
...
from routes.extract_url import _pin_safe_public_host, MAX_URL_FETCH_BYTES
from routes import extract_url as _extract_url_module


@pytest.fixture(autouse=True)
def _clear_pinned_hosts():
    """_pin_safe_public_host는 검사에 성공하면 그 주소를 스레드에 고정한다.
    테스트 사이에 남으면 다음 테스트가 낡은 주소를 보게 된다."""
    _extract_url_module._clear_pinned_hosts()
    yield
    _extract_url_module._clear_pinned_hosts()
...
# ---- _pin_safe_public_host: 순수 함수, 실제 네트워크 없이 검증 ----

def test_pin_safe_public_host_blocks_private_ranges():
    assert _pin_safe_public_host("127.0.0.1") is False
    assert _pin_safe_public_host("10.0.0.5") is False
    assert _pin_safe_public_host("192.168.1.1") is False
    assert _pin_safe_public_host("169.254.169.254") is False  # 클라우드 메타데이터
    assert _pin_safe_public_host("0.0.0.0") is False


def test_pin_safe_public_host_allows_public_ip():
    # DNS 조회 없이 바로 IP 리터럴을 넣어 네트워크 호출 없이 검증한다.
    assert _pin_safe_public_host("8.8.8.8") is True


def test_pin_safe_public_host_rejects_unresolvable():
    assert _pin_safe_public_host("this-domain-should-not-exist-xyz123.invalid") is False
...
async def test_extract_url_success():
    ...
    with patch("routes.extract_url.require_user_id", return_value="test_user_id"), \
         patch("routes.extract_url._pin_safe_public_host", return_value=True), \
