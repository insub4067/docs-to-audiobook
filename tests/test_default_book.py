...
def reset_default_book_state():
    ...
    original = dict(main.default_book_state)
    yield
    main.default_book_state.clear()
    main.default_book_state.update(original)


def test_fingerprint_changes_with_content_and_voice(tmp_path, monkeypatch):
    source = tmp_path / "book.md"
    source.write_text("원본 내용")
    monkeypatch.setattr(main, "DEFAULT_BOOK_SOURCE", str(source))
    monkeypatch.setattr(main, "DEFAULT_BOOK_VOICE", "voiceA")

    fp_original = main._default_book_fingerprint()
    assert fp_original.startswith("voiceA.")

    # 같은 입력이면 항상 같은 지문이어야 한다(캐시 키로 쓰이므로 결정적이어야 함)
    assert main._default_book_fingerprint() == fp_original

    # 원문 내용이 바뀌면 지문도 바뀌어야 한다 — 이게 실제로 고친 버그다.
    source.write_text("완전히 다른 내용으로 교체됨")
    fp_after_content_change = main._default_book_fingerprint()
    assert fp_after_content_change != fp_original

    # 음성이 바뀌어도 지문이 바뀌어야 한다.
    monkeypatch.setattr(main, "DEFAULT_BOOK_VOICE", "voiceB")
    fp_after_voice_change = main._default_book_fingerprint()
...
def test_paths_and_remote_keys_share_the_same_fingerprint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(main, "DEFAULT_BOOK_SOURCE", str(source))
    monkeypatch.setattr(main, "DEFAULT_BOOK_VOICE", "voiceA")
    monkeypatch.setattr(main, "DEFAULT_BOOK_DIR", str(tmp_path / "default_book"))

    fp = main._default_book_fingerprint()
    audio_path, meta_path = main.default_book_paths()
    remote_audio, remote_meta = main.default_book_remote_keys()
...
async def test_prepare_default_book_from_cache_disk_hit(tmp_path, monkeypatch, reset_default_book_state):
    ...
    result = await main.prepare_default_book_from_cache()

    assert result is True
    assert main.default_book_state["status"] == "ready"
...
async def test_prepare_default_book_from_cache_cloud_hit(tmp_path, monkeypatch, reset_default_book_state):
    ...
    monkeypatch.setattr(main, "DEFAULT_BOOK_DIR", str(tmp_path / "empty_default_book"))
    monkeypatch.setattr(main, "_restore_default_book_from_cloud", lambda a, m: True)
...
async def test_prepare_default_book_from_cache_miss(tmp_path, monkeypatch, reset_default_book_state):
    ...
    monkeypatch.setattr(main, "DEFAULT_BOOK_VOICE", "voiceA")
    monkeypatch.setattr(main, "DEFAULT_BOOK_DIR", str(tmp_path / "empty_default_book"))
    monkeypatch.setattr(main, "_restore_default_book_from_cloud", lambda a, m: False)
    ...
    assert main.default_book_state["status"] == "pending"
